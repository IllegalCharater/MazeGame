...
public static partial class ExcelToJsonExporter {
    ...
    private static void Export() {
        try {
            ...
            // 按扫描到的表 A1（rootKey）懒创建通用模块，无需注册表
            var modules = new Dictionary<string, ExcelExportModule>(StringComparer.Ordinal);
            foreach (var absPath in excelPaths) {
                ...
                foreach (DataTable table in ds.Tables) {
                    ...
                    if (string.IsNullOrEmpty(rootKey))
                        continue;
                    if (!modules.TryGetValue(rootKey, out var module)) {
                        module = new ExcelExportModule();
                        module.Init(rootKey);
                        modules[rootKey] = module;
                    }
    ...
    private static void GetExportStemMap() {
        ...
        string assetPath = getJsonPath(BaseJsonName);
    ...
    private static void WriteBaseJsonManifest() {
        WriteJson(getJsonPath(BaseJsonName), ExportJsonStemMap);
    }
}
