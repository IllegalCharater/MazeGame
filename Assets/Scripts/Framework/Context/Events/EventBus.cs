...
public sealed class EventBus {
    // 存储：事件枚举 -> 委托列表（支持任意签名）
    private readonly Dictionary<EventType, List<Delegate>> _handlers = new();
    // 事件绑定：父事件 -> 子事件列表（当父事件触发时，自动发布这些子事件）
    private readonly Dictionary<EventType, List<EventType>> _eventBindings = new();

    // ---------- 订阅 ----------
    public void Subscribe(EventType eventType, Delegate handler) {
    ...
    /// 支持多级绑定，但会检测循环依赖。
    /// </summary>
    public void Subscribe(EventType parentEvent, EventType childEvent) {
    ...
    // ---------- 取消订阅 ----------
    public void Unsubscribe(EventType eventType, Delegate handler) {
    ...
    /// 移除指定的一对父子绑定
    /// </summary>
    public void Unsubscribe(EventType parentEvent, EventType childEvent) {
        if (_eventBindings.TryGetValue(parentEvent, out var children)) {
    ...
    public void UnsubscribeAll(EventType eventType) {
        ...
        if (_eventBindings.TryGetValue(eventType, out var eventList)) {
            eventList.Clear();
            _eventBindings.Remove(eventType);
        }
    }
    ...
    public void Publish(EventType eventType, object[] args) {
        ...
        foreach (var handler in snapshot) {
            try {
                ...
                Debug.LogError($"EventBus 发布 {eventType} 异常: {ex.Message}");
            }
        }
    }

    // ---------- 循环依赖检测 ----------
    private bool IsCircularDependency(EventType parent, EventType child) {
        // 检测从 child 出发是否能回到 parent
        var visited = new HashSet<EventType>();
        return IsReachable(child, parent, visited);
    }
    private bool IsReachable(EventType from, EventType target, HashSet<EventType> visited) {
